...
void setup() 
{
  Serial.begin(115200);
  WiFi.disconnect();
  WiFi.begin("ITK-A.X","K@mpusM3rdeka!"); // nama wifi ssid dan Passwordnya
  while ((!(WiFi.status() == WL_CONNECTED))){
    delay(5000); 
  }
...
// Menghitung rata-rata kelembaban
  ...
  http.begin(client, "https://iot.itk.ac.id/monitoring_server/insert.php");
  ...
  request_string += msg;
  int httpResponseCode = http.POST(request_string);
